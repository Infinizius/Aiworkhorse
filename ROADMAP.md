# AI-Workhorse v8 – Entwicklungsstatus & Roadmap

> **Stand:** April 2026 | **Version:** `v1.1.1 (Hardened)` | **Branch:** `copilot/fix-bugs-roadmap-readme`
...
### 2.2 Offene Punkte (Phase 2) ⚠️

| Komponente | Status | Priorität |
|---|---|---|
| Frontend-Tests (vitest) | ⚠️ Noch nicht implementiert – Next.js App Router erfordert separaten Audit | 🔵 Phase 2 |
| CI/CD-Pipeline | Keine GitHub Actions, kein automatisches Deployment | 🔵 Phase 2 |
| JWT/OAuth2-Authentifizierung | Aktuell Bearer Token (statisch) – JWT mit Expiry für Phase 2 geplant | 🔵 Phase 2 |
| LangGraph-Agenten | Für Phase 2 geplant (`GOAL_MAX_ITERATIONS`) | 🔵 Phase 2 |
| HITL Cross-User-Manipulation | ✅ Behoben (v1.1.1): `execution_id` ist jetzt an `user_id` in Redis gebunden | ✅ Behoben |

### 2.3 Gesamtfortschritt (v1.1 Hardened)

```
Infrastruktur      ███████████████████████  ~100% (Caddy HTTPS, 6 Services inkl. Worker)
Backend-Logik      ███████████████████████  ~100% (Gemini/Mistral/DeepSeek, RAG, HITL, Caching)
Frontend / UI      ████████████████████░░░   ~90% (Dashboard, Dokumente-Seite, WebUI)
RAG-Pipeline       ███████████████████████  ~100% (Upload, Embed async, Retrieve, Inject)
Sicherheit         ██████████████████████░   ~95% (API-Key Auth, Encryption, HITL-Binding)
Tests              █████████████████████░░   ~90% (39 Backend-Tests; Frontend-Tests fehlen)
Dokumentation      ███████████████████████  ~100% (README, ROADMAP, Swagger, Security Audit)
─────────────────────────────────────────────────────────────────────────────────
Gesamt (v1.1)      ███████████████████████  ~97% GEHÄRTET – bereit für Produktion
```
...
### 3.3 Behobene Architekturprobleme (Post-v1.0 Audit) ✅
...
- **Fix:** 39 Backend-Tests implementiert (`test_health`, `test_security`, `test_chat`, `test_models`).
- Frontend-Tests (vitest) sind für Phase 2 vorgesehen.

#### ARCH-13: Kein Health-Endpoint – ✅ BEHOBEN (M7)
- **Fix:** `GET /readyz` (public, für Docker-Healthchecks) und `GET /health` (auth, DB + Redis Status).

#### ARCH-14: `GET /v1/files` gibt unvollständige Daten zurück – ✅ BEHOBEN (v1.1.1)
- **Problem:** API lieferte nur `file_id` und `filename`; Frontend erwartete außerdem `page_count`,
  `chunks_embedded`, `uploaded_at`, `preview` und `total`.
- **Fix:** `list_files`-Endpoint gibt jetzt alle Felder inkl. Embedding-Zählung zurück.

#### ARCH-15: `DEFAULT_MODELS` in `docker-compose.yml` war ungültig – ✅ BEHOBEN (v1.1.1)
- **Problem:** `gemini-3.1-flash-lite` existiert nicht in der `/v1/models`-Liste des Backends.
- **Fix:** Korrigiert auf `gemini-3-flash-preview`.

#### ARCH-16: `api`-Service hatte kein `depends_on` für `db` und `redis` – ✅ BEHOBEN (v1.1.1)
- **Problem:** Race Condition: API startete ggf. bevor DB/Redis bereit waren, was zu Startup-Fehlern führte.
- **Fix:** `depends_on` mit `condition: service_healthy` für `db` und `redis` hinzugefügt.

#### ARCH-17: IVFFlat-Index in Alembic-Migration `c1c21ee5d1e1` gelöscht aber nicht wiederhergestellt – ✅ BEHOBEN (v1.1.1)
- **Problem:** Die autogenerierte Migration hat den IVFFlat-Index für Vektor-Suche gelöscht (sequentieller Scan, langsam).
- **Fix:** Migration erweitert: Index wird nach dem Drop sofort per `CREATE INDEX IF NOT EXISTS` neu erstellt.

#### ARCH-18: HITL-Freigabe nicht an Nutzer gebunden – ✅ BEHOBEN (v1.1.1)
- **Problem:** Jeder authentifizierte Nutzer konnte fremde Tool-Freigaben approven (SECURITY_AUDIT Finding A).
- **Fix:** `execution_id` wird beim Start an die `user_id` in Redis gebunden (TTL 70s). `approve_tool` prüft die Bindung und gibt 403 zurück bei Mismatch.

### 3.4 Offene Architekturprobleme (Phase 2) 🔵

- **JWT/OAuth2:** Übergang von statischem Bearer Token zu ablaufenden JWT-Token.
- **Frontend-Tests:** Vitest-Suite für Next.js App Router.
- **CI/CD-Pipeline:** GitHub Actions für automatisierte Tests und Deployment.
- **LLM-Guardrails:** NLP-basierte Injection-Erkennung (z.B. Llama Guard) statt Regex.
